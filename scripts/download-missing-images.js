...
async function getImageUrlFromProductPage(productLink) {
  try {
    ...
    // Method 5: Try to find any image with "product" in the src
    if (!imageUrl) {
      $('img').each((i, el) => {
        const src = $(el).attr('src') || $(el).attr('data-src');
        if (src && (src.includes('/products/') || src.includes('cdn/shop'))) {
          imageUrl = src;
          return false; // break
        }
      });
...
async function downloadMissingImages() {
  ...
  const productsNeedingImages = products.filter(product => {
    // If no imageUrl at all
    if (!product.imageUrl) return true;
    
    // If it's a local path but file doesn't exist
    ...
    // If it's a remote URL, we should download it
  ...
  for (let i = 0; i < productsNeedingImages.length; i++) {
    ...
    if (!imageUrl && product.imageUrl && product.imageUrl.startsWith('http')) {
      ...
      console.log(`  Using existing remote URL: ${imageUrl.substring(0, 80)}...`);
    }
